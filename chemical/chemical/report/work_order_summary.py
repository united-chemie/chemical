...
def get_data(filters):
	...
	fields = ["name", "status", "sales_order", "production_item", "qty", "produced_quantity",
...
def get_chart_based_on_qty(data, filters):
	...
	for d in labels:
		pending.append(flt(periodic_data.get("Pending").get(d),2))
		completed.append(flt(periodic_data.get("Completed").get(d),2))
	...
	for from_date, end_date in ranges:
		...
		for d in data:
			if getdate(d.planned_start_date) >= from_date and getdate(d.planned_start_date) <= end_date:
				periodic_data["Pending"][period] += (flt(d.qty) - flt(d.produced_quantity))
				periodic_data["Completed"][period] += flt(d.produced_quantity)
